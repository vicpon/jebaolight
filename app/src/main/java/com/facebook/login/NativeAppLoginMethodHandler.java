...
abstract class NativeAppLoginMethodHandler extends LoginMethodHandler {
    ...
    private Result handleResultOk(Request request, Intent data) {
        ...
        if (extras.get(NativeProtocol.BRIDGE_ARG_ERROR_CODE) != null) {
            errorCode = extras.get(NativeProtocol.BRIDGE_ARG_ERROR_CODE).toString();
        } else {
            Object errorCode2 = result;
        }
        ...
        if (error == null && errorCode == null && errorMessage == null) {
            try {
                ...
                return Result.createErrorResult(request, "", ex.getMessage());
            }
        } else if (ServerProtocol.errorsProxyAuthDisabled.contains(error)) {
            return result;
        } else {
            if (ServerProtocol.errorsUserCanceled.contains(error)) {
                return Result.createCancelResult(request, "");
            }
